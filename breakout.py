#в этом классе реализуется управление игры, по этому он и назван в честь самой игры
import random
...
assert os.path.isfile('sound_effects/brick_hit.mp3')
...
#создание меню
    def create_menu(self):
        ...
        def on_quit(button):
            ...
            self.is_game_running = False
            self.game_over = True
# При нажатии кнопки PLAY вызывается функция on_play(), удаляющая кнопки из списка objects, чтобы они
# больше не отрисовывались. Кроме того, значения булевых полей, которые запускают начало игры — is_game_running
# и start_level — становятся равными True.
# При нажатии кнопки QUIT is_game_running принимает значение False (фактически ставя игру на паузу),
# а game_over присваивается значение True, что приводит к срабатыванию последовательности завершения игры.
...
#генерируем мячик
    ...
    def create_bricks(self):
        ...
        self.bricks = bricks
# с помощью этой функции мы распознаем ударился ли мяч о что либо, в этом нам помогает intersect
    def handle_ball_collisions(self):
        ...
        # удар об пол (потеря жизни или конец игры)
        ...
        # удар об киприч (1 очко, проверяется все кирпичи, чтобы определить какой ударился об мяч)
...
#необходимая преостановка игры, нужна для того чтобы игрок смог прочитать нужное сообщение и ничего не отвлекало
